...
class CreateGroupPost extends StatefulWidget {
  final String docId4;
  const CreateGroupPost({Key? key, required this.docId4}) : super(key: key);
  @override
  CreateGroupPostState createState() =>
      CreateGroupPostState(docId4: this.docId4);
}
...
class CreateGroupPostState extends State<CreateGroupPost> {
  //for passing group id
  String docId4;
  CreateGroupPostState({required this.docId4});
  //for sharing
  File? imageFile;
  ...
  Widget build(BuildContext context) {
    CollectionReference sharing =
        FirebaseFirestore.instance.collection('sharingGroups');
    ...
    Future<void> addSharing() async {
      ...
      return sharing
          ...
          .then((value){
            print("sharing added");
            FirebaseFirestore.instance.collection('groups').doc(docId4).update({'sharingId': FieldValue.arrayUnion([value.id])});
            // sharingId = value.id;
          })
